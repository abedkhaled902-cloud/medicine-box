...
function renderMedicines(list) {
  ...
  container.innerHTML = '';
  if (list.length === 0) {
    container.innerHTML = '<p class="text-center text-gray-500 col-span-3">暂无药品，点 + 添加第一盒药～</p>';
    return;
  }
  const now = new Date();
  list.forEach(med => {
    ...
    const card = `
      <div class="border rounded-xl p-6 shadow hover:shadow-lg transition ${bg}">
        ...
        <div class="mt-6 flex gap-3 flex-wrap">
          <button onclick="useMedicine('${med.id}', '${med.get('name')||'此药'}')" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-sm">用药记录</button>
          <button onclick="editMedicine('${med.id}')" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-sm">编辑</button>
          <button onclick="deleteMedicine('${med.id}')" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 text-sm">删除</button>
        </div>
      </div>`;
...
document.addEventListener('DOMContentLoaded', () => {
  loadMedicines();
  setInterval(loadMedicines, 10000); // 10秒轮询一次，够用
});
